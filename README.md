...
- 🎯 Entry points to execute your program using `python -m <flask_wpp_api>` or `$ flask_wpp_api` with basic CLI argument parsing.
...
---
# flask_wpp_api Flask Application

Awesome flask_wpp_api created by josuelopes512
...
```bash
git clone https://github.com/josuelopes512/flask_wpp_api flask_wpp_api
cd flask_wpp_api
make install
```
...
```bash
flask_wpp_api create-db   # run once
flask_wpp_api populate-db  # run once (optional)
flask_wpp_api add-user -u admin -p 1234  # ads a user
flask_wpp_api run
```
