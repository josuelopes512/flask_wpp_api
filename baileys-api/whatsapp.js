...
const createSession = async (sessionId, isLegacy = false, res = null) => {
    ...
    wa.ev.on('contacts.set', ({ contacts }) => {
        if (isLegacy) {
            store.contacts.insertIfAbsent(...contacts)
        }
    })
...
const getAllSession = () => {
    ...
    for (let session of sessions)
        if (session)
            active.push(session[0]);
        else
            deactively.push(session[0]);
    return { active, deactively };
}
...
const isJidStatus = async (session, jid) => {
    try {
        let result

        if (session.isLegacy) {
            result = await sock.fetchStatus(jid)
        } else {
            ;[result] = await sock.fetchStatus(jid)
        }
